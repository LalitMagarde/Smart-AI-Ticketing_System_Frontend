...
import { asyncGetTicket, asyncUpdateTicket, asyncUpdateTicketStatus } from '../../store/action/ticketAction';
import { asyncGetAllUsers } from '../../store/action/userActions';
...
const TicketDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchTicket = async () => {
      try {
        setIsLoading(true);
        const fetchedTicket = await asyncGetTicket(id);
  ...
  const handleSave = async () => {
    try {
      ...
      if (user.role==='admin' && status=='') {
        ...
        updated = await asyncUpdateTicket({id: ticket._id , info});
      }
      ...
      if (user.role!='user') {
        ...
        updated = await asyncUpdateTicketStatus({id:ticket._id,info});
      }
  ...
  return (
    <div className="bg-slate-800 shadow-xl rounded-lg overflow-hidden max-w-4xl mx-auto">
      <div className="px-6 py-5 border-b border-slate-700 flex justify-between items-center">
        ...
        { user.role!=='user' &&  <Button onClick={() => setEditMode(true)}>Edit</Button>}
      </div>
