...
const RoleBadge = ({ role }) => {
  ...
  return (
    <span className={`px-2 py-1 text-xs font-semibold rounded-full capitalize ${colors[role]}`}>
      {role}
    </span>
  );
};
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                    <div className="flex flex-wrap gap-1">
                      {user.skills?.map((skill) => (
                        <span key={skill} className="px-2 py-0.5 text-xs bg-slate-700 rounded-md">
                          {skill}
                        </span>
                      ))}
                    </div>
